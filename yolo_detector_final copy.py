...
def ret_verification(decision=verify):
    return decision
...
def yolo_core_and_children(model_core, model_child, image, detection_treshold, radar_pixels=None, width=640, height=640, bias = 0, draw=False, time_del=0, draw_detected_pts = False, classes=[0,2]):
    ...
    # sprawdzenie nakladania sie dziei
    confidences_children_list_2 = []
    boxes_children_list_2 = []
    detected_classes_children_list_2 = []
    
    dzieci_lb = len(detected_classes_children_list)
    already_detected_2 = [False for i in range(0, dzieci_lb)]
    if dzieci_lb > 1:
        for ropatrywane_dziecko in range(1, dzieci_lb):
            best_ratio = 0
            for dziecko in range(0, k):
                if detected_classes_children_list[dziecko] == detected_classes_children_list[ropatrywane_dziecko]:
                    ratio = bb_percentage_of_area(boxes_children_list[ropatrywane_dziecko], boxes_children_list[dziecko])
                    if ratio > best_ratio and ratio > 0.75:
                        best_ratio = ratio
                        already_detected_2[ropatrywane_dziecko] = True
                        score = max(confidences_children_list[dziecko], confidences_children_list[ropatrywane_dziecko])
                        confidences_children_list[dziecko] = score
    
    for i in range(0, dzieci_lb):
        if already_detected_2[i] == False:
            confidences_children_list_2.append(confidences_children_list[i])
            boxes_children_list_2.append(boxes_children_list[i])
            detected_classes_children_list_2.append(detected_classes_children_list[i])
    ...
    dane_dzieci = [boxes_children_list_2, detected_classes_children_list_2, confidences_children_list_2]
...
def check_core_and_x(detected_core_classes, boxes_core, classes_x, boxes_x):
    core_num = len(detected_core_classes)
    consensus_core_detections = [False for i in range(0, core_num)] # ktore rdzenie wykryte
    associated_x = [0 for i in range(0, core_num)] # ktory x zwiazany
    if core_num > 0:
        for rozpatrywane_r in range(0, core_num):
            best_ratio = 0
            ind_r = 100
            ind_x = 100
            for rozpatrywane_x in range(0, len(classes_x)):
                if detected_core_classes[rozpatrywane_r] == classes_x[rozpatrywane_x]:
                    ratio = bb_percentage_of_area(boxes_core[rozpatrywane_r], boxes_x[rozpatrywane_x])
                    if ratio > best_ratio and ratio > 0.6:
                        best_ratio = ratio
                        ind_r = rozpatrywane_r
                        ind_x = rozpatrywane_x
            if ind_r < 100:
                consensus_core_detections[ind_r] = True
                associated_x[ind_r] = ind_x
                
    return consensus_core_detections, associated_x


def calculate_values(conf_x, class_x, conf_core, class_core, conf_fusion, class_fusion, cars, people, movement, movement_real, conf_people, conf_cars, wrong_car, wrong_people, consensus_detections, associated_x):
    ...
    wrong_car_core, wrong_car_fusion, wrong_car_x = wrong_car[0], wrong_car[1], wrong_car[2]
    wrong_people_core, wrong_people_fusion, wrong_people_x = wrong_people[0], wrong_people[1], wrong_people[2]
    ...
    core_people_ok = True
    if detected_core_people > people:
        print("Detected {} people using core detector".format(detected_core_people), " real: {}".format(people))
        core_people_ok = False
    
    #detected_core_people = min(detected_core_people, people)
    #detected_fusion_people = min(detected_fusion_people, people)
    fusion_people_ok = True
    if detected_fusion_people > people:
        print("Detected {} people using fusion detector".format(detected_fusion_people), " real: {}".format(people))
        fusion_people_ok = False
    
    x_people_ok = True
    if detected_x_people > people:
        print("Detected {} people using x detector".format(detected_x_people), " real: {}".format(people))
        x_people_ok = False
    ...
    core_cars_ok = True
    if detected_core_cars > cars:
        print("Detected {} cars using core detector".format(detected_core_cars), " real: {}".format(cars))
        core_cars_ok = False
    
    #detected_fusion_cars = min(detected_fusion_cars, cars)
    fusion_cars_ok = True
    if detected_fusion_cars > cars:
        print("Detected {} cars using fusion detector".format(detected_fusion_cars), " real: {}".format(cars))
        fusion_cars_ok = False
    
    x_cars_ok = True
    if detected_x_cars > cars:
        print("Detected {} cars using x detector".format(detected_x_cars), " real: {}".format(cars))
        x_cars_ok = False
    #detected_x_cars = min(detected_x_cars, cars)

    verify_ = ret_verification()
    if verify_ or (not x_cars_ok) or (not core_cars_ok) or (not fusion_cars_ok) or (not x_people_ok) or (not core_people_ok) or (not fusion_people_ok):
        
        print("GIVE number of wrong detections:")
        print('Note: there are {} car children'.format(detected_fusion_cars - detected_core_cars))
        print('Note: there are {} people children'.format(detected_fusion_people - detected_core_people))
        for i in range(0, 5):
            print('CARS:')
            wrong_car_core_s = input('Give wrong number of car detections for core : ')
            wrong_car_fusion_s = input('Give wrong number of car detections for fusion : ')
            wrong_car_x_s = input('Give wrong number of car detections for x : ')
        
            print('People: ')
            wrong_people_core_s = input('Give wrong number of people detections for core : ')
            wrong_people_fusion_s = input('Give wrong number of people detections for fusion : ')
            wrong_people_x_s = input('Give wrong number of people detections for x : ')
            
            try:
                wrong_car_core_t = int(wrong_car_core_s)
                wrong_car_fusion_t = int(wrong_car_fusion_s)
                wrong_car_x_t = int(wrong_car_x_s)
                
                wrong_people_core_t = int(wrong_people_core_s)
                wrong_people_fusion_t = int(wrong_people_fusion_s)
                wrong_people_x_t = int(wrong_people_x_s)
                break
            except ValueError:
                print("ogarnij się!")
                pass
        wrong_car_core += wrong_car_core_t
        wrong_car_fusion += wrong_car_fusion_t
        wrong_car_x += wrong_car_x_t
        
        wrong_people_core += wrong_people_core_t
        wrong_people_fusion += wrong_people_fusion_t
        wrong_people_x += wrong_people_x_t
    ...
    wrong_car = [wrong_car_core, wrong_car_fusion, wrong_car_x]
    wrong_people = [wrong_people_core, wrong_people_fusion, wrong_people_x]

    
    return movement_real, Confidence_metric_x, Confidence_metric_core, Confidence_metric_fusion, Detected_x, Detected_core, Detected_fusion, conf_people, conf_cars, wrong_car, wrong_people
...
for k in range(0, num_run):
    wrong_car = [0, 0, 0]
    wrong_people = [0, 0, 0] # core, fusion, x
    file_count = 0
    total_time = 0
    point_count = 0
    point_count_red = 0
        ...
        if file_count % (2 + labels_exist) == 0:
            ...
            if x is None: # nie wykryto nic przez radar
                ...
                if mode == 0:
                    ...
                    rdzen_boxes, rdzen_classes = rdzen[0], rdzen[1]
                    consensus_detections, associated_x = check_core_and_x(rdzen_classes, rdzen_boxes, detected_classes_x, boxes_x)
                    ...
                    people_count += people
                    car_count += cars
                    movement_real, Confidence_metric_x, Confidence_metric_core, Confidence_metric_fusion, Detected_x, Detected_core, Detected_fusion, conf_people, conf_cars, wrong_c, wrong_p = calculate_values(confidences_x, detected_classes_x,
                                                                                                                                                                                                rdzen[2], rdzen[1], fuzja[2], fuzja[1],
                                                                                                                                                                                                cars, people, movement, movement_real,
                                                                                                                                                                                                conf_people, conf_cars,
                                                                                                                                                                                                wrong_car, wrong_people,
                                                                                                                                                                                                consensus_detections, associated_x)
                    wrong_car = wrong_c
                    wrong_people = wrong_p
...
print('WRONG NUMBERS')
print('CAR: =')
w_car_core, w_car_fusion, w_car_x = wrong_car[0], wrong_car[1], wrong_car[2]
w_people_core, w_people_fusion, w_people_x = wrong_people[0], wrong_people[1], wrong_people[2]
print('wrong core: ', w_car_core)
print('wrong fusion ', w_car_fusion)
print('wrong x: ', w_car_x)
print('HUMANS: =')
print('wrong core: ', w_people_core)
print('wrong fusion ', w_people_fusion)
print('wrong x: ', w_people_x)
